...
public class AdminAnalyticsServiceImpl implements AdminAnalyticsService {
    ...
    public AnalyticsSummaryResponse getSummary() {
        ...
        long total = outfitRepository.count();
        long thisWeek = outfitRepository.countByCreatedAtGreaterThanEqualAndCreatedAtLessThan(weekAgo, now);
        long lastWeek = outfitRepository.countByCreatedAtGreaterThanEqualAndCreatedAtLessThan(twoWeeksAgo, weekAgo);

        LocalDate today = LocalDate.now();
        LocalDateTime dailyFrom = today.minusDays(6).atStartOfDay();
        Map<String, Long> dailyCounts = toCountMap(
                outfitRepository.countDailySince(dailyFrom),
                0
        );
        ...
        Map<String, Long> monthlyCounts = toCountMap(
                outfitRepository.countMonthlySince(monthlyFrom),
                0
        );
